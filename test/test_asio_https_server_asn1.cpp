...
int main(int argc, char** argv) {
  ...
  if (cert_der.empty() || key_der.empty() || cert_pem.empty() || key_pem.empty()) {
    std::cerr << "usage: test_asio_https_server_asn1 --port <p> --cert-der <cert.der> --key-der <key.der> --cert <cert.pem> --key <key.pem>\n";
    return 2;
  }
  ...
  try {
    ...
    ctx.use_private_key(asio::buffer(key), asio::ssl::context::asn1, ec);

    // Keep the in-memory ASN.1 code paths exercised, but configure a known-good
    // server identity for the roundtrip itself.
    ...
    std::cerr << "asio https asn1 server failed: " << ex.what() << "\n";
    return 1;
  }
